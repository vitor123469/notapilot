...
export async function POST(request: Request) {
  ...
  if (!reason) {
    return apiError(422, "E_REASON", "Motivo do cancelamento é obrigatório.");
  }
  ...
  if (nfse.status !== "authorized") {
    return apiError(409, "INVALID_STATUS", "Só é possível cancelar NFS-e com status authorized.");
  }
  if (!nfse.provider_nfse_number) {
    return apiError(422, "E_NO_NUMBER", "NFS-e sem número do provider para cancelamento.");
  }
  ...
  if (requestUpdateError) {
    return apiError(500, "NFSE_UPDATE_FAILED", "Falha ao marcar cancel_requested.");
  }
  ...
  if (requestedEventError) {
    return apiError(500, "EVENT_INSERT_FAILED", "Falha ao registrar evento CANCEL_REQUESTED.");
  }

  const providerResult = await mockNfseNacionalProvider.cancel({
    tenantId,
    nfseId,
    providerRequestId: nfse.provider_request_id ?? undefined,
    providerNfseNumber: nfse.provider_nfse_number,
    reason,
  });
  if (providerResult.status !== "cancelled") {
    return apiError(422, providerResult.errorCode ?? "CANCEL_REJECTED", providerResult.errorMessage ?? "Cancelamento rejeitado.");
  }
  ...
  const { error: cancelUpdateError } = await admin
    ...
    .update({
      ...
      raw_response: providerResult.rawResponse ?? nfse.raw_response,
      error_code: null,
      error_message: null,
    })
    .eq("tenant_id", tenantId)
  ...
  if (cancelUpdateError) {
    return apiError(500, "NFSE_UPDATE_FAILED", "Falha ao marcar NFS-e como cancelled.");
  }

  const { error: cancelledEventError } = await admin.from("nfse_events").insert({
    tenant_id: tenantId,
    nfse_id: nfseId,
    event_type: "CANCELLED",
    payload: { providerNfseNumber: nfse.provider_nfse_number },
  });
  if (cancelledEventError) {
    return apiError(500, "EVENT_INSERT_FAILED", "Falha ao registrar evento CANCELLED.");
  }
  ...
  if (auditError) {
    return apiError(500, "AUDIT_LOG_FAILED", "Falha ao registrar auditoria do cancelamento.");
  }
